...
// Tính điểm cục bộ của vùng (x, y, k) và biên xung quanh
int get_local_score(int field[MAXN][MAXN], int n, int x, int y, int k) {
    int score = 0;

    // Cặp ngang trong vùng k×k
    for (int i = y; i < y + k; i++) {
        // Bên trong vùng
...
int calculate_delta_score(int field[MAXN][MAXN], int n, int x, int y, int k) {
    ...
    // Tính điểm mới từ submatrix đã xoay
    int new_score = 0;
...
// --------- SMALL SOLVER (n < 20) trả về Result ----------
Result local_search_small(const int start_field[MAXN][MAXN], int n, int time_limit) {
    ...
    while (true) {
        ...
        if (no_improve > 3000) {
            if (current_score > max_pairs * 0.95) {
                base_accept_bad = min(0.2, base_accept_bad * 3.0);
            }
...
// --------- LARGE SOLVER (n >= 20) trả về Result ----------
Result local_search_solver_large(const int start_field[MAXN][MAXN], int n, int time_limit) {
    ...
    while (true) {
        ...
        while (picked < samples && tries_limit > 0) {
            ...
            if (active_size > 6 && um.any) {
                if (!move_touches_unmatched(um, x, y, k)) {
                    continue;
                }
            }
